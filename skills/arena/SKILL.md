...
3. Pick the runners. Spawn one candidate per model available to the Agent tool (`opus`, `sonnet`, `fable`, `haiku`). Mixing model families is the point: different families fail differently. Spawn more when the arena covers multiple design directions. Same model N times when the work is generation-bound rather than judgment-sensitive.
4. Assign output paths. Each candidate writes to its own location (a git worktree where possible, otherwise `/tmp/arena-<slug>/candidate-<n>/`). N candidates writing to one path is shared mutable state: they clobber each other.
...
The synthesized artifact has to hold up under the same scrutiny as any other output, and the way you show that is by running it. The arena does not earn you a pass.
...
Adaptado de las skills originales de pstack (MIT). Se quitaron las referencias a
`~/.cursor/rules/pstack-models.mdc` y a las "principle skills" de pstack, que no
existen en Claude Code.

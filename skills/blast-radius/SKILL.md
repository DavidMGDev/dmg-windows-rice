...
Reading the code tells you what it does. Reading its history tells you why it's shaped that way. Blast radius tells you what it breaks somewhere else.
...
1. Read the change. The diff, the symbols it adds, changes, and deletes, and what it now does differently, including the part the diff doesn't spell out. Pull the PR and commits with `gh` and `git log -p`.
...
Write it through `no-ai-slop`, cite real code, and strip anything private before it goes anywhere public.
